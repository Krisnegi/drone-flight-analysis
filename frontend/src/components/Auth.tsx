    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 text-slate-100">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-6 text-center text-3xl font-extrabold text-slate-100 tracking-tight">
          {isLogin ? 'Sign in to Flight Control' : 'Register Pilot Credentials'}
        </h2>
        <p className="mt-2 text-center text-sm text-slate-400">
          ...
          >
            {isLogin ? 'request new pilot registration' : 'sign in with existing key'}
          </button>
        </p>
      </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            {!isLogin && (
              <>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-300">Security Clearance / Role</label>
                  <div className="mt-2 grid grid-cols-2 gap-4">
                    <button
                      type="button"
                      onClick={() => setRole('PILOT')}
                      className={`py-3 px-4 border rounded-xl flex items-center justify-center space-x-2 transition-all ${
                        role === 'PILOT'
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300">Security Access Key (Password)</label>
              <input
                type="password"
                required
                value={password}
            ...
            <div>
              ...
              >
                {loading ? 'Processing telemetry...' : isLogin ? 'Access Control System' : 'Provision Pilot Key'}
              </button>
            </div>
          </form>
        </div>
